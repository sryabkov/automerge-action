...
function checkReady(pullRequest, context) {
  ...
  if (["mergeable_state"].include(config.mergeableProperty)) {
    return mergeable(pullRequest, context);
  } else if (["mergeable"].include(config.mergeableProperty)) {
    return mergeableAlt(pullRequest, context);
  } else {
    throw new Error(
      `Invalid mergeableProperty: ${config.mergeableProperty}`
    );
  }
}
